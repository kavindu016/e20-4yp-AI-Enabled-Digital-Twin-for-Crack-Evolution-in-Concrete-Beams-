...
public class SegmentedBeamGenerator : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        Debug.Log("Generating beam mesh once with segments = " + segments);

        Mesh mesh = GenerateSegmentedBeam(segments, length, height, width);

        // IMPORTANT: allow runtime modification
        mesh.MarkDynamic();

        mf.mesh = mesh;

        Debug.Log("Mesh assigned → vertex count = " + mesh.vertexCount);
    }
    ...
    Mesh GenerateSegmentedBeam(int seg, float L, float H, float W)
    {
        ...
        Debug.Log("Mesh built → vertices = " + vList.Count + " triangles = " + tList.Count / 3);
